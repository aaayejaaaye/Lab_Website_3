...
app.get('/player_info', function(req, res){
	var play_id = req.query.player_choice;
  var query1= 'select id, name from football_players;';
  var query2 = 'select * from football_players where id=' + play_id + ';';
  var query3 = 'select count(*) from football_games where ' + play_id + ' = any(players);';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query1),
          task.any(query2),
